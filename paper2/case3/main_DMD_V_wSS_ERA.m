...
%% Build The measurement matrix
 rows =round(0.35*length(y));% 2500 gives bad result  <------------------------------------------------------------
 m = length(y)-rows;
...
if k==ss
   legend('Signals','DMD' ,'rDMD','MP','ERA')
end
end
%  legend('Measurements','DMD')
% axis([40 60.4 203.1 207.3])
title('Event 3: Algorithm Comparisons')
...
Error_rdmd=sum(abs(y-X_rdmd(1:ss,:)));
Error_mp=sum(abs(y-X_MP'));
Error_era=sum(abs(y-X_ERA'));
