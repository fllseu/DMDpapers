...
function [eig_s, y_est] = fun_prony_rankreduction(ya, dT, m, np)
%clear; clc; 
%load ymat

%dT = 0.001;  m=10;
% multiple-channel prony analysis
% y: each column a channel
% dT: sampling rate
% m:  system order
[N, n_ch] = size(ya);
...
%m = 3; 
if(m > np)
   disp('order should be less than np');  
end
...
[re, ind]=sort(abs(imag(eig_a1)),'ascend');

eig_s = eig_a1(ind(1:m));
z = exp(eig_a1(ind(1:m))*dT);
